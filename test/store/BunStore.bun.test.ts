...
import { BunStore } from "../../src/store-bun/BunStore.js";

describe("BunStore", () => {
  let tempDir: string;
  let store: ReturnType<typeof BunStore.create>;

  beforeAll(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "bunstore-test-"));
    store = BunStore.create(tempDir);
